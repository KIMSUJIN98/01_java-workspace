...
public class B_ArrayCopy {
	...
	//배열 복사
	public void method1() {
		int[] origin = {1,2,3,4,5}; // 선언, 대입, 할당을 한번에 한다
	...
	public void method2() {
		...
		int[] copy = new int[5]; // new int[origin.length]도 가능, 현재 메모리 내에는 0 0 0 0 0 값이 저장된 상태
		
		/*
		참고사항 : 배열의 길이 < 배열의 크기 선언 : 75번 라인에서 에러 발생: 할당해줄 origin[5~9] 값이 미존재 //int[] copy = new int[8];
				배열의 길이 > 배열의 크기 선언 : 배열의 크기만큼만 저장된 배열 생성 //int[] copy = new int[3];
		*/
		...
		*/
		...
		for(int i =0; i<copy.length; i++) {
			copy[i] = origin[i]; // 메모리 내 값이 0 0 0 0 0에서 1 2 3 4 5로 변경 할당됨.
		}
	...
	public void method3() {
		//2. 아싸리 새로운 배열 생성 후 System이라는 클래스에서의 arraycopy() 메소드를 이용한 복사 : 인덱스 지정가능, 크기지정 별도로 가능
		int[] origin = {1,2,3,4,5};
		int[] copy = new int[10]; //넉넉하게 크게 생성 / 0~9 인덱스까지 0이 다 담겨있을 것!!
		
		//System.arraycopy(원본배열명, 복사시작할 인덱스(원본의 인덱스), 복사본배열명, 복사본 배열의 복사될 시작인덱스, 복사할갯수); //메소드 카멜표기법 미적용
		
		//System.arraycopy(origin, 0, copy, 0, 5); // 1 2 3 4 5 0 0 0 0 0
		//System.arraycopy(origin, 0, copy, 2, 5); // 0 0 1 2 3 4 5 0 0 0 ; 커스터마이징이 가능하다
	...
	public void method4() {
		// 3. Arrays 클래스 제공하는 copyOf() 메소드 사용 : 인덱스 지정불가, 크기지정 가능
		int[] origin = {1,2,3,4,5};
	...
	/*
	 * Arrays.copyOf() 메소드
	 * => 내가 제시한 길이 만큼 복사본 배열 크기 할당 (할당 = heap 영역에 새로운 방이 생김)
	 ...
	 * java.lang.String
	 * java.lang.System
	 * => java.lang 패키지에 있는 클래스들을 쓸때는 import 안해줘도됨.
	 * 
	 * java.util.Scanner
	 * java.util.Arrays
