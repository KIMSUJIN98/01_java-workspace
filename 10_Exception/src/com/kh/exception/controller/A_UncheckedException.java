...
public class A_UncheckedException {
	...
	public void method1() {
		...
		*/
		...
		try {
			...
			e.printStackTrace(); // 강제로 오류난 이력 보고자 할 때 실행하는 구문 								// 에러가 난게 아니라 보여주기만 함, 따라서 아래 구문들도 실행 가능
		}
	...
	public void method2() {
		...
		*/
		...
		}catch(ArrayIndexOutOfBoundsException e) {													// catch는 에러 개수에 따라 다중작성 가능, 범위가 다르므로 매개변수 중복 가능
		...
		//System.out.println("100번 인덱스의 값 : " + arr[100]); 										// (size <= 100 경우) ArrayIndexOutOfBoundsException 발생
	...
	public void method3() {
		...
		}catch(NegativeArraySizeException e) {														// java.lang에 있으므로 별도의 import가 필요없다
			System.out.println("배열의 크기로 음수를 제시할 수 없습니다.");
		}catch(ArrayIndexOutOfBoundsException e) {													// java.lang에 있으므로 별도의 import가 필요없다
			System.out.println("부적절한 인덱스로 접근했습니다.");
		}catch(InputMismatchException e) { // import => java.util									// java.util에 있으므로 별도의 import가 필요하다 
		...
		*/
		...
		}catch(InputMismatchException e) {															// RuntimeException 아래에 있게되면, RuntimeException이 먼저 낚아채므로 InputMismatchException 까지 도달하지 못한다.
			System.out.println("정수가 아닌값이 들어간거같음..");
		}catch(RuntimeException e) { // 다형성 적용해서 부모타입예외클래스 작성가능
		 ...
		 * 부득이하게 조건문으로 핸들링이 안되는 구문 예외처리 구문으로 작성해 둘 것!!
		 */
	}
