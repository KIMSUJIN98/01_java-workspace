...
public class MapRun {
	...
	public static void main(String[] args) {
		...
		// 1. put(K key, V value) : 컬렉션에 키 벨류 세트로 추가시켜주는 메소드									// 회원가입(추가), ID를 key값으로
		...
		// 저장되는 순서 유지 안됨!! value 값이 중복되더라도 키값이 중복이 안되면 잘 저장됨!!
		...
		// 2. get(Object key) : V => 컬렉션에서 해당 키값을 가지는 Value값을 반환시켜주는 메소드					// 회원정보 가져오기
		...
		// 4. replace(K key, V value) => 컬렉션에서 해당 키값을 찾아서 다시 전달한 value값으로 수정시켜주는 메소드		// 비밀번호 변경
		hm.replace("포테이토칩", new Snack("겁나 짠맛", 1000));
		System.out.println(hm);
		
		// 5. remove(Object key) => 컬렉션에서 해당 키값을 찾아서 그 키-벨류 세트를 삭제시켜주는 메소드					// 회원탈퇴
		hm.remove("포테이토칩");
		System.out.println(hm);
		...
		// ArrayList는 안됨
		// ArrayList list = new ArrayList(hm);														// value만 담을 수 있는데, key값이 포함되므로
		
		// Iterator 반복자를 이용!
		// Iterator it = hm.iterator(); // 곧바로 iterator() 호출 못함!! 왜? iterator()는 List계열 Set계열에서만 쓸 수 있는 메소드임!
