...
public class B_StringMethodTest {
	...
	public void method() {
		...
		String str2 = str1.concat("!!!");		// StringPool에 들어가지 않는다
		System.out.println("str2 : " + str2);
		
		String str3 = str1 + "!!!";				// .concat() 함수와 같은 역할 // StringPool에 들어간다
		System.out.println("str3 : " + str3);
		
		System.out.println("str2와 str3가 일치합니까? " + (str2 == str3));
		// 근데 아까전에 StringPool에서 같은 글자면 같은 주소아닌가요?
		// concat 열어보면 new 있음 => 새로운 주소값 heap
		// str3만 StringPool 에 있음!
		...
		// 6. 문자열.substring(int beginIndex) : String => 문자열의 beginIndex 위치에서 부터 끝까지의 문자열 추출
		...
		// ** Quiz
		// 1. str1 => Worl만 추출 + concat() !를 붙여서 Worl! => str0
		...
		System.out.println("str1이 변경됐나? : " + str1); 										// 원본은 변경되지 않음
		...
		System.out.println("Lower : " + str5.toLowerCase());								// hello world

		/*
		 * System.out.print("계속하시겠습니까? (y/n) : ");
		 * char ch = sc.nextLine().toUpperCase().charAt(0); // 'N', 'Y'
		 * 
		 * char ch = sc.nextLine().charAt(0).toUpperCase();
		 * 뒤에써도 되나요? 안됨!!!! => String 클래스에서 제공하는 함수임!!
		 * 
		 * if(ch = 'N') {
		 *  // 프로그램 종료
		 * }
		...
		String str6 = "   JA  VA   "; 					// 앞뒤 세칸의 공백만 지워지고, 문자열 중간에 있는 2칸의 공백은 제거되지 않는다.
		System.out.println("str6 : " + str6.trim());	// 예시) 사용자에게 id, pwd 받을때 공백 제거해서 받는다.
